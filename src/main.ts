...
// Fallback modal for malformed/partial tables
import { Modal } from 'obsidian';

class FallbackTableModal extends Modal {
  private tableLines: string[];

  constructor(app: App, tableLines: string[]) {
    super(app);
  ...
  onOpen() {
    ...
    const title = contentEl.createEl('h3', { text: 'Table Editor Unavailable' });
    title.style.marginBottom = '8px';

    const msg = contentEl.createEl('div', { text: 'This table is malformed or partial and cannot be opened in the rich editor. You can copy the raw Markdown below to fix or paste elsewhere.' });
    msg.style.marginBottom = '12px';
    msg.style.fontSize = '14px';
    msg.style.color = '#b91c1c';

    const textarea = contentEl.createEl('textarea');
    textarea.value = this.tableLines.join('\n');
    ...
    copyBtn.onclick = () => {
      navigator.clipboard.writeText(textarea.value).then(() => {
        new Notice('Table Markdown copied to clipboard.');
      });
    };
...
import { RichTableModal } from './rich-table-modal';
import { extractTableAtCursor, serializeTable } from './markdown-table-utils';
...
export default class TableEditorPlugin extends Plugin {
  ...
  public async onload(): Promise<void> {
    ...
    // Rich Table Editor Command
    this.addCommand({
      id: 'open-rich-table-editor',
      name: 'Open Rich Table Editor',
      editorCallback: (editor: Editor, view: MarkdownView) => {
        const cursor = editor.getCursor();
        const lines = editor.getValue().split('\n');
        const table = extractTableAtCursor(lines, cursor.line);
        if (!table) {
          new Notice('Cursor is not in a Markdown table.');
          return;
        }
        // Validate table structure: must have at least header, separator, and consistent columns
        const isValidTable = (tbl: typeof table): boolean => {
          if (!tbl.rows || tbl.rows.length < 2) return false;
          const colCount = tbl.rows[0].length;
          if (colCount === 0) return false;
          // Check all rows have same column count
          if (!tbl.rows.every(row => row.length === colCount)) return false;
          // Check for separator row (---) in second row
          const sepRow = tbl.rows[1];
          const isSeparator = sepRow.every(cell => /^-+$/.test(cell));
          if (!isSeparator) return false;
          return true;
        };
        if (!isValidTable(table)) {
          // Fallback: show raw table in a modal with copy option
          new FallbackTableModal(this.app, lines.slice(table.tableStartIdx, table.tableEndIdx + 1)).open();
          return;
        }
        const modal = new RichTableModal(this.app, table.rows, table.colWidths, (updatedRows, updatedWidths) => {
          // Serialize table back to Markdown
          const metaLine = table.metaLineIdx !== undefined ? lines[table.metaLineIdx] : null;
          const newLines = serializeTable(updatedRows, updatedWidths, metaLine);
          // Replace lines in editor
          editor.replaceRange(
            newLines.join('\n'),
            { line: table.metaLineIdx !== undefined ? table.metaLineIdx : table.tableStartIdx, ch: 0 },
            { line: table.tableEndIdx, ch: lines[table.tableEndIdx].length }
          );
        });
        modal.open();
      },
